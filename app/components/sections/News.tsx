...
const NewsCard = ({ title, image, category = "NEWS" }: NewsCardProps) => (
  <div
    ...
    style={{
      ...
      boxShadow: '0 8px 32px rgba(0, 0, 0, 0.3), inset 0 0px 0 rgba(255, 255, 255, 0.2)',
    }}
  >
    ...
    <div className="flex h-32 relative z-10">
      ...
      <div className="flex-1 p-6 pr-4 flex flex-col justify-between">
        <h3 className="text-white font-bold text-lg leading-tight group-hover:text-white/90 transition-colors drop-shadow-lg">
          {title}
        </h3>

        <div className="flex items-center gap-2">
          <Newspaper className="w-4 h-4 text-white/70 drop-shadow-sm" />
          <span className="text-white/70 text-xs font-semibold tracking-wider uppercase drop-shadow-sm">
            {category}
          </span>
        </div>
      </div>
      ...
      <div className="w-48 h-32 relative flex-shrink-0 overflow-hidden rounded-r-xl">
        ...
        />
        {/* Image overlay for better glass integration */}
